...
class GaborConv2d(nn.Conv2d):
    """ A simple 2d Convolution initialized
    """

    def __init__(self, *args, frequency, theta=0, bandwidth=1, sigma=None, sigma_x=None, sigma_y=None, n_stds=3, offset=0, **kwargs):
        """ Gabor-initialized convolution operation.

        For parameter description, see:
            https://scikit-image.org/docs/dev/api/skimage.filters.html#skimage.filters.gabor_kernel
        """
        if len(args >= 3):
            kernel_size = args[2]
        else:
        ...
        if sigma is not None:
            sigma_x = sigma
            sigma_y = sigma
        kernel = np.real(gabor_kernel(frequency=frequency, theta=theta, bandwidth=bandwidth,
